# 更新履歴
 ...
 - trunk/HEAD
 - 4.4.7 (2026.9.25)
   * タブバー末尾にタブが追加された場合に、タブがタブバー端に張り付いた状態にならずスクロール範囲外に隠れてしまうことがあったのを修正
   * [Bug 1542044](https://bugzilla.mozilla.org/show_bug.cgi?id=1542044)の暫定回避策として、暗い配色のウィンドウにそぐわない明るい配色が適用されることを防げるよう、ブラウザーのテーマの配色をサイドバーに適用しない設定を追加した（[by Robertof, thanks!](https://github.com/piroor/treestyletab/pull/3972)）
   * ブラウザー自身のNovaテーマで使われている色をより正しく取り扱えるようにした（[by Robertof, thanks!](https://github.com/piroor/treestyletab/pull/3972)）
   * API由来の脆弱性に対する回避策として使えるよう、「他の拡張機能向けAPIを有効化する」オプションをろ見えやすい位置に露出させるようにした
 - 4.4.6 (2026.9.18)
   * `get-tree` および `get-light-tree` の脆弱性を修正：タブの完全な情報が意図せず他のアドオンに公開されてしまっていました。この問題は4.0.0から4.4.5までのすべてのバージョンが影響を受けます。
 - 4.4.5 (2026.9.18)
   * 最新のNightlyに基づいてNovaテーマを更新
   * Novaテーマとシステムカラーの組み合わせに対応
   * 「新しいタブ」ボタンのクリック時の振る舞いとして、常にコンテナーを選択してからタブを開く選択肢を追加
   * コンテナー選択や新規タブの関係を指定して開くメニューについて、アンカー要素に紐付かないときはマウスカーソルの近くにメニューを開くようにした
   * コンテナー選択メニューの「新しいコンテナーを追加」および「コンテナーを管理」機能を再現するようにした
   * ユーザースタイルをタブのプレビューやタブグループのメニューなどコンテンツ領域内のUIにも適用するようにした
   * 新規ウィンドウを開いたときのタブの再訴の表示に失敗する場合があったのを修正
   * コンテナー用の円形アイコンが表示されていなかったのを修正
 - 4.4.4 (2026.9.2)
   * 「（タブを）新しいウィンドウへ移動」で開かれるウィンドウのサイズや配置をFirefox側に任せるようにした
   * ピン留めされたタブが上下に跳ねるような動きが発生しないよう、ピン留めされたタブが一行だけのときはスクロールが発生しないように修正
   * ピン留めされたタブの領域のリサイザーの幅を、より操作しやすくなるよう拡大した
   * タブバー端に張り付いたタブの背景画像がより正しく表示されるようにした
   * 最新のNightlyに追従してNovaテーマの色定義を更新
 - 4.4.3 (2026.8.27)
   * 設定画面において無関係の項目にブラウザー設定に関する権限の説明があったのを修正
 - 4.4.2 (2026.8.27)
   * Firefox 155以降でもNovaではなくProtonを規定のテーマとして使うようにした
   * ブラウザーのテーマで定義された色をより適切に反映するようにした
   * browserSettings権限を必須ではなく任意にした
 - 4.4.1 (2026.8.26)
   * Novaテーマ選択時にブラウザーのテーマをより適切に反映するようにした
   * ピン留めされたタブ用の領域のリサイザーの外観をFirefoxネイティブの物に近付けた
   * ピン留めされたタブ用の領域をリサイズできなくなっていたのを修正（4.4.0での後退バグ）
 - 4.4.0 (2026.8.26)
 ...
 - 4.3.5 (2026.7.3)
   * Protonテーマの外観をFirefoxネイティブの垂直タブに近付けた（ピン留めしたタブのサイズが拡大されましたが、ユーザースタイルシートで `:root { --faviconized-tab-size: 28px; }` と指定すると従来のサイズに戻せます）
   * コンテキストメニューの「共有」の動作をFirefox本体の物に近付ける形で再構築した
   * Firefox本体に追従して、タブのURLを共有するためのQRコードを生成できるようにした
   * キーボードショートカットを「タブ（ツリー）を先頭（末尾）に移動」の操作に割り当て可能にした（初期状態では未設定のため、手動で設定する必要があります）
   * 並列で再読み込みするタブの最大数を制限するようにした（既定値は3で、上級者向け設定で変更可能）
   * アンロードされたタブの復帰・再読み込みを可能な限り順番に行うようにした
   * 親タブを閉じた時にグループ化用のタブで置き換える設定のとき、グループ化用のタブのタイトルを閉じられた親タブのタイトルにするようにした
   * ウィンドウ間を移動したツリーの構造をより正しく維持するようにした
   * ツリー構造の復元をより堅牢にした（[by tkng](https://github.com/piroor/treestyletab/pull/3926), thanks!）
   * ユーザープロファイル内のセッション情報やタブグループの情報が破損している場合でもより堅牢に動作するようにした
   * ユーザースタイルシート編集中に意図せず編集結果が失われる問題を起こりにくくした
   * 設定の編集・同期・リセット時にメッセージ処理の無限ループが発生する可能性があったのを修正
 - 4.3.4 (2026.4.27)
   * 「リンクを分割ビューで開く」で開かれた部活ビューが正しく表示されない問題を修正
   * ブックマークフォルダー作成時に入力した名前が使われない問題を修正（4.3.3の後退バグ）
   * ESR140などいくつかの環境で独自のダイアログが空のウィンドウになってしまっていたのを修正（4.3.1の後退バグ）
   * 独自のダイアログが適切なサイズで開かれていなかったのを修正（4.3.1の後退バグ）
   * スクロールなどで大量のタブが連続して再描画されたときのパフォーマンス低下を解消
 - 4.3.3 (2026.4.17)
   * 並べ替えた後のタブ内の要素（カウンターなど）の状態が更新されなくなる問題を修正（4.3.0の後退バグ）
   * 並べ替えた後のタブの表示位置の計算（タブが可視範囲内にあるかどうかの判定や、アクティブなタブを可視範囲内に入れるためのスクロール動作など）がおかしくなっていたのを修正
   * 分割ビューのタブが含まれない場合でも、折りたたまれたタブグループに無意味な「+0」というラベルが表示されないようにした
 - 4.3.2 (2026.4.13)
   * キャッシュが使用されない場合でも、折りたたまれたツリー内のタブの内部状態をより正しく復元するようにした
   * Firefoxと同様に、サイドバー上端部でのドロップ位置の表示時に「タブをここにドロップしてピン留めします」という説明を表示するようにした
   * macOS上のFirefox日本語版において「新規タブ」コマンドで開かれたタブをより確実に検出できるようにした
   * 「このタブをブックマーク」のダイアログの内容がタブのタイトルとURLで埋まっていない状態で開かれる問題を修正（4.3.1の後退バグ）
   * サイドバーの端に貼り付けられたタブのアイコンが意図せず消去されてしまう問題を修正（4.3.0の後退バグ）
   * アクティブなタブを閉じた後に、待機状態（メモリーからアンロードされた状態）のタブにフォーカスが移らないようにする動作について、グループ化用タブには例外的にフォーカス可能とする上級者向け設定を追加した
   * コンテキストメニューの「リンクをコピー」機能のラベルについてzh_CNの翻訳を追加
   * Webページ内で実行されるスクリプトでハンドリングされていない例外が報告される量を削減
 - 4.3.1 (2026.4.3)
   * 最近のバージョンのFirefoxで独自のダイアログ（複数タブを閉じる際の確認、ブックマーク作成など）が動作しなくなっていたのを修正
   * ドラッグ＆ドロップ操作時の不要な設定変更の監視処理を減らした
   * アンロード状態のタブを複製した後にタブがサイドバー内で非表示になってしまう問題を修正
   * API: `get-tree` および `get-light-tree` で対象のタブを `tabId` や `tabIds` で指定した場合に結果が返らなくなっていたのを修正
 - 4.3.0 (2026.3.26)
 ...
 - 4.2.11 (2026.3.3)
   ...
   * 複数のタブを一度に閉じる際に確認を行う設定が有効な場合でも、Firefox自身の動作と同様に、閾値の25個（`browser.sessionstore.max_tabs_undo`の既定値に同じ。この設定を変更した場合は閾値の数値をそれに同期させてください）より少ない数のタブがまとめて閉じられる場面では確認を省略するようにした（by [tkng](https://github.com/piroor/treestyletab/pull/3856), thanks!）
 ...
 - 4.2.8 (2026.1.9)
   * Firefoxのネイティブの機能と同様に、折りたたまれたグループについてコンテンツ領域上にカスタムツールチップを表示するようにした。
   * 折りたたまれたツリーについてコンテンツ領域上に表示するカスタムツールチップにおいて、タブのアイコンを表示するようにした上で、クリックされた項目に対応するタブへの切り替えを可能とした。
   * 現在のタブのコンテンツ領域内に表示する、折りたたまれたツリーやグループのためのカスタムツールチップ について、現在のタブのコンテンツ領域内への表示ができない場合に、初期状態でサイドバー内に表示するようにした。
   * ピン留めされたタブとそうでないタブの境界付近でのタブのドラッグ＆ドロップ時の振る舞いを、Firefoxネイティブのタブバーの動作に近付けた（ピン留めされていないタブを最後のピン留めされたタブの直後にドロップしたらピン留めし、ピン留めされたタブを最初のピン留めされていないタブの直前にドロップしたらピン留め解除するようにした）
   * グループの項目の外観をFirefoxネイティブのタブバー上の物により近付けた
   * アクティブになったタブがグループの最後のメンバーだった場合に、折りたたまれたグループのメンバーの数を表示するカウンターが正しく表示されなかったのを修正
   * グループを他のグループより前の位置にドラッグ＆ドロップしたときに正しく移動できるようにした
   * グループ内で子タブをドラッグ＆ドロップしたときにツリー構造が壊れないようにした
   * グループの項目のドラッグ中は、ドロップしたタブをピン留めする事を示す領域を表示しないようにした
   * data: URIで提供されたPNG形式のアイコンが不正なデータたった場合に、TSTのUI上では壊れた画像のアイコンを表示しないようにした
 - 4.2.7 (2025.10.21)
