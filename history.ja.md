# 更新履歴
 ...
 - master/HEAD
   * faviconの読み込みに関連する無限再帰呼び出しの問題を修正
   * ツリー上のフォーカスを移動する既定のショートカット「Ctrl-Shift-矢印」が文字入力フィールドで一般的に使われているショートカットと衝突していたため削除した
   * すべてのキーボードショートカットを割り当て解除できるようにした（[bug 1475043](https://bugzilla.mozilla.org/show_bug.cgi?id=1475043)の回避策として、「Reset」ボタンで初期状態のショートカットを復元する代わりにショートカットを消去するようにした）.
